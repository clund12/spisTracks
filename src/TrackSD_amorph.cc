...
G4bool TrackSD::ProcessHits(G4Step* step, G4TouchableHistory*)
{
    ...
    *************************************************************************************************/
    ...
    G4double vel = step->GetTrack()->GetVelocity();
...
void TrackSD::EndOfEvent(G4HCofThisEvent*)
{
    ...
    for ( const auto& radius : radii )
    {
        ...
        *accMan->GetAccumulable<G4double>(wAvName) += wAv;
        ...
        if ( (rHitPos.x()*rHitPos.x() + rHitPos.y()*rHitPos.y()) <= 0.0116*trackVel*0.0116*trackVel )
        {
            yFName = "yF_a"+counterString;
            yDName = "yD_a"+counterString;
        ...
        {
            yFName = "yF_d"+counterString;
            yDName = "yD_d"+counterString;
